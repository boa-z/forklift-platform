...
- **音频与亮度在设备上真实生效**：声音经 PocketJS 宿主 audio 模块（`aplay` 子进程 +
  环形缓冲，pak 里的 `audio:wav.btn`），亮度经宿主 backlight 模块写
  `/sys/class/backlight`；宿主未挂载模块时（开发/模拟）两者降级为空操作。
  设置里的音量滑条作用于提示音播放器（软音量）。最终形态仍由 daemon 后端接管，
  平台 API 与协议不变。
